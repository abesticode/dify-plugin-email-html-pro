...
def send_mail(params: SendEmailToolParameters) -> Dict[str, Tuple[int, bytes]]:
    ...
    # Handle raw HTML mode - content is sent as-is without any processing
    if params.is_raw_html:
        # For raw HTML, the email_content is already HTML, send it directly
        # Plain text fallback is just the raw content stripped of tags (basic fallback)
        import re
